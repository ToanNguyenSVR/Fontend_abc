  ...
  return (
    <div className="job">
      <ManagerTemplate
        ...
        modalContent={
          <>
            <Steps current={current} items={items} className="job-step" />
            {steps.map((item, index: number) => {
              return (
                <div
                  style={contentStyle}
                  className={current === index ? 'step-content active' : 'step-content'}
                >
                  {item.content}
                </div>
              );
        ...
        handleDelete={id => myAxios.delete(`job/${id}`)}
        handleCreate={(form: any) => {
          const outputArray: any[] = [];
          for (const key in form) {
            if (form.hasOwnProperty(key)) {
              const match = key.match(
                /recruiterStages\[(\d+)\]\.(createDate|finishDate|rewardPercent|personQuantity|recruitStageId)/
              );
              if (match) {
                ...
                outputArray[index][field] = form[key];
                delete form[key];
              }
            }
          }

          form.recruiterStages = outputArray;
          form.staff = user.companyStaff?.id;
          form.image = selectedImage;

          return myAxios.post(`job`, form);
        }}
        handleUpdate={(id: number, form: any) => {
          ...
          for (const key in form) {
            if (form.hasOwnProperty(key)) {
              const match = key.match(
                /recruiterStages\[(\d+)\]\.(createDate|finishDate|rewardPercent|personQuantity|recruitStageId|stageId)/
              );
              if (match) {
                const index = parseInt(match[1]);
                const field = match[2];
          ...
          form.staff = user.companyStaff?.id;
          form.image = selectedImage;
          return myAxios.put(`job/${id}`, form);
        }}
        handleUpload={(record: any) =>
          myAxios.get(`/confirm-pay?jobRequest=${record.recruiterStages[0]?.stageId}`)
        }
        fetchData={() => myAxios.get(`/job/${user.company?.id}`)}
