...
const CVForm: FC<CVFormProps> = ({ cv, file, disable }) => {
  const [form] = useForm();
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (user.email != values['email']) {
        toast.error('This CV now belong to you!');
        return;
      }

      const response = await myAxios.post(`candidate/${user.candidate?.id}`, payload);
  ...
  return (
    <div className="cv-form">
      ...
      {current > 0 && (
        <Button style={{ margin: '5px 8px' }} onClick={() => prev()}>
          Previous
        </Button>
      )}

      {current < steps.length - 1 && (
        <Button style={{ margin: '5px 8px' }} type="primary" onClick={() => next()}>
          Next
        </Button>
      )}
